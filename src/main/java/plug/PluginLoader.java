...
public class PluginLoader {
  ...
  /**
   ...
   public List<Class<IPlugin>> getPluginClasses() {
    List<Class<IPlugin>> res = new ArrayList<Class<IPlugin>>(loadedPluginClasses.values());
    logger.info("$$$$$$$$$$$$$$$$$$$$$$$$"+res+"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
     return res;      
  }
  ...
  private void loadFromDirectory() {
    logger.info("=+=+=+=+=+ Entry in loadFromDirectory=+=+=+=+ I will load from " + pluginDirectory );
  ...
  private void loadFromSubdirectory(File dir, String baseName) {
	  ...
	  logger.info("Le listing : " + Arrays.toString(files));
  ...
  private static String getQualifiedName(int baseNameLength, String classPath) {
    ...
    String res = classPath.substring(0, classPath.lastIndexOf('.')); 
	  // removing .class at end
	  logger.info("Fully qualified name of the class: " + res);
	  return res;
  }
	  ...
	  try {
		  ...
		  if (ptype.isAssignableFrom(loadedClass) ) {
			  ...
			  logger.warning("Class " + className +
					  ...
					  ptype.getName() + " but " + loadedClass.getName());
		  }
	  }
